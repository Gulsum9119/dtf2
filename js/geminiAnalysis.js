async function analyzeWithGemini(imageDataURL) {
  ...
  spinner.classList.remove('hidden'); card.classList.add('hidden'); errEl.classList.add('hidden');
  if (!key) {
  ...
  try {
    ...
    displayAnalysis(JSON.parse(jsonMatch[0]));
  } catch (e) {
    spinner.classList.add('hidden');
...
async function testAPIs() {
  ...
  resultEl.classList.remove('hidden'); resultEl.textContent = 'Проверка...';
  const results = [];
  if (geminiKey) {
    try { const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${geminiKey}`); results.push(r.ok ? '✅ Gemini API: OK' : `❌ Gemini API: HTTP ${r.status}`); }
    catch { results.push('❌ Gemini API: Ошибка сети'); }
  } else results.push('⚠️ Gemini API: Ключ не указан');
  results.push(removeBgKey ? '⚠️ remove.bg: Проверка только при использовании' : '⚠️ remove.bg: Ключ не указан');
  if (replicateKey) {
    try { const r = await fetch('https://api.replicate.com/v1/models', {headers:{'Authorization':`Token ${replicateKey}`}}); results.push(r.ok ? '✅ Replicate API: OK' : `❌ Replicate API: HTTP ${r.status}`); }
